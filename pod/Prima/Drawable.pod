=for rcs $Id: Drawable.pod,v 1.13 2002/11/18 14:03:36 dk Exp $
...
state, a color value is transformed to a system color representation, and
...
If 1, the Latin-1 encoded characters are assumed. 

The result is an integer array reference, where every character
glyph is referred by three integers, each triad containing
A, B and C values.
...
If UTF8 text is passed, unicode line break characters C<0x2028> and C<0x2029> are also
in effect.
...
resulting line. Used for efficiency when a reverse function to C<get_text_width>
is needed.
