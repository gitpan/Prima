=for rcs $Id: X11.pod,v 1.2 2003/12/03 17:17:51 dk Exp $
...
globbing, the use can set up a property value that corresponds to multiple
targets:
...
are given as C<PropetyClass.propertyname> index.
...
Xft font syntax is inherited from C<fontconfig> library and to be consulted
from C<man fonts-conf>, but currently ( November 2003 ) basic font descriptions
...
properties that belongs to two classes by means of standard XRDB techniques:

=over

=item class C<Foreground>

Widget properties: C<color>, C<hiliteBackColor>, C<disabledColor>, C<dark3DColor>

=item class C<Background>

Widget properties: C<backColor>, C<hiliteColor>, C<disabledBackColor>, C<light3DColor>
...
only. Theoretically, and paletted display on a monochrome monitor can be
...
640x480 (EGA) mode.  This visual is rarely met.
...
is the documented ( see L<Prima::Application> ) way to initiate UTF8 clipboard
text exchange.
...
Raw X11 provides no such GUI helpers and double-click event, cursor, or menu.
...
synchronization ( at expense of operation slowdown ) is activated with
C<--sync> command-line argument, and can be used to ease eventual debugging.
