=for rcs $Id: Widget.pod,v 1.5 2002/05/14 13:22:32 dk Exp $
...
maps onto screen as a rectangular area,
with disctinct boundaries, pointer and sometimes
...
heavily, providing the programmer easy and unified
access to the system-generated events, that occur
when the user moves windows, clicks a mouse,
...
The functionality of the class is much more extensive
...
becomes a top-level widget. Because of that, the C<insert>
...
C<create()> call. In addition, another notification, a C<Setup>
message is sent after the widget is created. This message is
I<posted>, so it is called within C<create()> but processed
in the application event loop. It means that C<Setup> execution
...
When the life time of a widget is over, C<destroy()> is called,
...
and the owner afterwards. In such situaltion, widget can operate with 
...
A notification responsible for repainting a widget is C<Paint>.
...
can be or can be not within the C<repaint()> call. This behavior is governed by
the C<::syncPaint> property.
C<repaint()> marks the whole widget's area as to be refreshed, as the I<invalidated>
area. For the finer gradation of the area that should be repainted,
C<invalidate_rect()> and C<validate_rect()> pair of functions is used. 
...
NB: Further assignments of C<::clipRect> property do not
...
The direct rendering, contratry to the event-driven,
is started by a program, not by a system. If a programmer
wishes to paint over a widget immediately, then 
C<begin_paint()> is called, and, if successfull, the part 
...
that is like if the user sweeps a window over the progress bar
widget. The direct method does cannot be that smart, but if the
status bar is the insignificant part of the program, the trade-off
functionality vs code simplicity might be considered acceptable.
...
functionalities. The base properties are C<::origin> and C<::size>,
...
postion and size. The notifications occur irrespectible to whether the
...
extenstion will be adjusted to the boundary values, not to the
specified ones.
...
corresponds to C<::growMode> 0, widget does not change its size
...
conditions. A widget with size-dependant C<::growMode> ( with
...
Example: child widget has width 100, gm::GrowHiX
growMode and sizeMin set to (95, 95). Its owner has width 200. 
...
NB: Strictly speaking, the I<virtual size> must be declared a read-only property,
...
module, which assigns a random font as the default one, so the testing
phase does not involve tweaking of the system settings.

=head1 Z-order 

In case two widgets overlap, one of these is drawn
...
This set of features grants an owner the I<parent> term. However,
...
widget do not transgress implictly to the owner's top-level window
...
A widget belonging to an application object, can not reset its C<::clipOwner>
values to 1.

The C<::clipOwner> property opens a possiblity for the toolkit widgets
...
the widget becames child to this window, which can be any application
...
is never destroyed before its C<Close> notfication grants the destruction.
...
all previously dicussed implicit sizing issues. When change
...
NB. It must be noted, that the reverse is true: if a widget
...
These relations between current-mark and focus allow the toolkit
...
focus the widgets with C<::selectable> set to 0.

=head2 Mouse-aided focusing

The typical behavior of the mouse is when the user left-clicks
on a widget, it becomes focused.
...
circulation. If it doesn't, C<next_tab()> never uses it in its algorithm.
...
tab-focus candidate is picked. The default C<::tabOrder> value is -1,
...
Keyboard input generates two major notifications - C<KeyDown>
...
hints that the characted code is of value. Some other C<kb::XXX>-marked keys
...
but with the price of under-optimization. If the core C<KeyDown> processing
...
Mouse notifications are send in response to the user 
that moves the mouse or presses or releases its buttons.
...
The mouse input polling is full-weight, and along with C<get_mouse_state()>
that returns C<mb::XXX> constant combination, the C<::pointerPos> 
...
explicitly set, the owner link breaks and C<::ownerColor> or 
C<::ownerBackColor> automatically set to 0.
...
property, a multiplexer for all eight color preoperties. C<ci::> constants
mimic their non-RGB C<cl::> counterparts, so the call
C<hiliteBackColor(cl::Red)> is equal to C<colorIndex(ci::HiliteText, cl::Red)>.
...
constants are descriped in L<"API">; their usage can make
...
method, that often deals with system-dependend and user-selected
preferences ( see L<"Additional resources"> ). Because a widget hold
...
In gross, the C<::pointer> property is enough for these actions.
...
and C<hide_cursor()>, but these are not the convenience shortcuts
...
or C<::showHint> are explicitly set, the owner link breaks and 
C<::ownerHint> or C<::ownerShowHint> are automatically set to 0.
...
descentants - Prima::AccelTable and Prima::Popup ( Prima::Window
is also with Prima::Menu). The C<::items> properties of these objects
...
fucntion ends here. C<::popup> provides access to a context pop-up menu,
...
The change to font and color properties for a menu object is not always granted.
The underlying system capabilities in this area range
from total inability for a program to manage the menu fonts and colors 
in Win32, to a sport for the user changing menu fonts and colors
dynamically, with the immediate results in OS/2.
...
Maintains the lower Y boundary of a widget. If changed,
...
enough system resources ( memory is the one ), then widget
draws in the usual manner.
...
the mouse events would not pass the widget the mouse pointer
is over, but are redirected to the widget. The call for capture
may not be always granted due the race conditions 
between programs.
...
( and any other ) policy, and is not ornamented by the system-specific 
decorations.

Default value: 1

See L<"Parent-children relationship">
...
Specifies the lower left corner of a cursor
...
A color used to draw dark shadings of 3D-looking widgets.
...
a widget ( usually serving as a dialog or a grouping 
widget ) was designed.
...
A color used to substitute C<::backColor> when a widget 
...
A color used to substitute C<::color> when a widget 
is in its disabled state.
...
if the top-level window it belongs to was not activated, so to
select such a widget it takes two mouse clicks.
...
Disregards C<::selectable> property.
...
where 'manpage' is a file with POD content and 'section'
is a topic inside the manpage.

See also: C<help>

=item hiliteBackColor COLOR

A color used to draw alternate background areas
with high contrast.
...
A color used to draw alternate foreground areas
with high contrast.
...
Maintains the left X boundary of a widget. If changed,
...
A color used to draw light shadings of 3D-looking widgets.
...
Maintains the left X and bottom Y boundaries of a widget relative
...
operations. See L<Prima::Drawable/"palette"> section.
...
then a widget becomes the child of a window specified,
given the wigdet's C<::clipOwner> is 0.
...
Specifies the kind of the pointer, associated with a widget.
...
pointers, their icons and hot spot offsets. C<cr::User> is a general sign that
...
when the user right-clicks or selects an appropriate keyboard
combination. Prima::Widget can host many children objects,
and Prima::Popups as well. But only the one that is set in
...
Maintains the right X boundary of a widget. If changed,
does not affect the widget width; but does so, if called
...
a widget. If 0, it is forbidden. The C<::hint>
...
the occupier is assigned another unique value, but without desctruction
...
indicator. Negative value is never returned.
...
Maintains the upper Y boundary of a widget. If changed,
does not affect the widget height; but does so, if called
...
does not clears the background during C<Paint> event.
...
class that is defined by system and is associated with Prima::Widget
...
A write-only property. Once set, widget is centered by the verical
axis relative to its owner.
...
Calls C<can_close()>, and if successfull, destroys a widget.
...
with the widget's owner C<::helpContext> strings,
...
=item mouse_wheel MOD = 0, X = 0, Y = 0, Z = 0, POST = 0 
...
state of widget area covered by the rectangula area passed. If a widget
...
X and Y are mouse pointer coordinates when
the session started.
...
Called when a hint label is about to show or hide,
...
pressed when the event occured ( C<km::XXX> ). REPEAT
...
Sent to the focused widget when the user depresses a key.
CODE contains an eventual character code, KEY is one of C<kb::XXX>
constants, MOD is a combination of the modificator keys
pressed when the event occured ( C<km::XXX> ).

See also: C<KeyDown>, C<key_up>

=item Leave

Called when the input focus leaves a widget
...
depressed within system-defined interval of time ) is
