=for rcs $Id: tutorial.pod,v 1.4 2005/01/26 10:35:23 dk Exp $
...
The idea behind the complexity is to tell what exactly the menu item it should be by
...
The other types (text,image,sub-menu) are told by looking at the type of scalars they
...
The actions taken when the menu item is selected, is always a pointer to the code. It is possible
to either set an anonymous subroutine, as it is shown in all the examples above, or to set
a string. The latter case will cause the method of the menu owner ( in this example, the
window ) to be called. This can be useful when constructing a generic class with menu
...
All actions are called with the menu item identifier as a parameter.

Another trick is to define a hot key. While its description can be arbitrary, the
hot key can be constructed in two ways. It is either a literal such as C<^A> for 
Control+A, or C<@B> for Alt+B, or C<^@#F10> for Control+Alt+Shift+F10. Or, alternatively,
it is a combination of C<km::> constants either with ordinal of the character letter or
the key code, the latter is one of C<kb::> constants. The latter is less readable, but more
