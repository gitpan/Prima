=for rcs $Id: Window.pod,v 1.3 2002/05/14 13:22:32 dk Exp $

=head1 NAME

Window and Dialog - top-level window management
...
Prima::Widget can never be clipped by anything but the
screen, and that the system or window manager add
...
windows, and some time these rules are hard or even impossible to 
circumvent. It will be a long document, describing the features
...
and test programs with as many as possible. 
...
not be combined - the value of C<::borderStyle> is one of the constants.
...
on the task bar, by operating the C<::taskListed> property.
...
But there is at least one point of certainity.
...
with certainity. This problem results in that it is impossible to
...
the user to work with the application windows until it is closed.
There can be other message boxes on top of each other, preventing
the lower window from operation as well.
This is an exclusive modality.
...
Any window, called execute() is set on top of the last exclusive
...
as of its owner. Here introducted the 'modal horizon'
term. A modal horizon is a root of the shared window stack, 
and the default horizon is $::application's. The boolean C<::modalHorizon>
...
calling C<end_modal()> method. The window is then set hidden and disabled, 
and the windows below can be operated. If the window was in exclusive
modal state, the execute() call finished and returns an exit code, which
can be set through C<::modalResult> property. This property is an convenience
...
These are C<ok()> and C<cancel()> methods; the latter is identical to
when the user closes the modal window by the means of the system.
The values set to C<::modalResult> are C<mb::OK> and C<mb::Cancel>,
correspondingly. There are more C<mb::XXX> constants, but these represent
...
of the menu. The syntax of the items-deribed properties is described in
L<Prima::Menu>, but it must be reiterated that menu item records only
...
NB: A window can be owner for several menu objects and still do not
have a menu bar, if no menu objects are marked as selected.
...
and resemble the system dialogs - on systems where they are
provided.
...
Reflects whether a window is a root to the shared modal window stack
or is a non-root. A window with C<::modalHorizon> set to 1
in shared modal state groups its children windows in a separate
hierarchy from other shared modal hierarchies. The C<::modalHorizon>
is therefore useful only when several shared modal window stacks
are needed.

The property also serves as an additional grouping factor
for widgets and windows. For example, default keyboard navigation by
tab and arrow keys is limited to the windows and widgets of a horizon.
...
A standart method to dismiss a modal window with C<mb::Cancel>
...
A standart method to dismiss a modal window with C<mb::OK>
result. Typically the effect of calling this method is equal to when
the user presses the enter key of a modal window, signalling that
