=for rcs $Id: Menu.pod,v 1.2 2002/05/14 13:22:32 dk Exp $

=head1 NAME

Menu - pull-down and pop-up menu objects
...
An unique string, assigned to every menu item. Menu items can be
acceessed individually by their names, and the following fields
can be managed by calling elemental properties, that require an item
name. The name itself can be dynamically changed by C<set_variable()>
method. If the menu name was not given explicitly, it is formed 
by C<sprintf("MenuItem_%d", unique number)> string automatically.
...
This syntax is valid only for C<::items> and C<insert()> functions,
not for C<set_variable()> method.
...
usually accerator text is 'Ctrl+Enter' string. 
...
in responce to Prima::Widget's C<Popup> notification.
...
decsendant provided with different extra functionalites.
...
implict hot-key lookup when on C<KeyDown> event.
...
is an arbitrary string, with '~' ( tilda ) quotation
...
NB: Prima::AccelTabel use no system resources, and
...
Disables or enables menu items, associcated with
...
Translates three-integer key prepresentation into
