=for rcs $Id: image-load.pod,v 1.6 2002/09/25 13:31:21 dk Exp $
...
Simpliest case, loading a single image would look like:
...
Contains an anonymous array of frame indeces to load. 
Valid indeces are above zero, negative ones can't be counted in a way
perl array indeces are. Example:
...
Typical output, for example, from a gif codec based on libunguf would look
like:
...
signalling that codec is incapable of quick reading of the frame count.
...
group - this information is to be found in codec documentaion;
...
the request was issued on. This behavor can be altered 
...
Save requests also can be performed with package syntax:
   ...
   die "$@" unelss Prima::Image-> save( 'filename.duf', 
       images => [ $x]);

=head2 Multiframe saving

In multiframe invocation save returns number of successfully saved frames.
File is erased though, if error occured, even after some successfully 
written frames.
...
Actual correspondence between codecs and their indeces is described latter.
...
for selection of an error distribution algorithm, if downsampling 
is required.
...
an index in this array. Indeces for a codecs registered once never change,
...
always interestring to know, what part it is.
