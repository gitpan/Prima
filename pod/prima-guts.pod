...
the OO concepts, but in short, because Prima is written on C, not on C++,
...
An object can be automatically destructed when its reference count
is reached 0. Note that the auto destruction would never happen
...
destroyed explicitly, but it's reference would still live, 
...
in re-entrant fashion.

=head2 Data instance

All object data after the creation represent an object instance.
All Prima objects are blessed hashes, and the hash key
__CMATE___ holds a C pointer to a memory which is occupied
by C data instance, or "mate". It keeps all object variables
...
call_perl_indirect() function accepts object, it's method name
...
or array ) value is returned. The caller is responsible of 
...
To avoid this, use bracketign functions called C<protect_object()> 
and C<unprotect_object()>.
...
after every routine that calls perl, because object might be destroyed
inside this call. Typical code example would be like:
...
C mate even for objects in csDead stage, use C<gimme_the_real_mate()>
function instead.

=head2 init

Object's method init() is responsible for setting all initial properties to 
...
is used, and pdelete() is used for key deletion, alhough is it not
...
precautions to be taken into account when an object gets created from C code.
As an example would serve a piece of real code from DeviceBitmap.c:
...
otherwise it might be destroyed before it's time.
...
object value would be assigned to a scalar, for exmaple.
Thus
...
ant it's descendants, and also for Timer, AbstractMenu, Printer
...
this way the call success flag is set, and first paramster have to 
...
If type conversion is performed along with palette change, some effeciency
...
__ORDER__ anonymouse array have to be added to set() parameters.
...
On error condition, concatenates $@ with value stored in
dG_EVAL_ARGS variable
...
Returns a C pointer to an object, if perlObject is a reference to a
Prima object. Same as C<gimme_the_mate>, but does not care about
object's stage.
...
If action returns true, enumeration stops and the value is returned.
Otherwise nil is returned. action have to be function prototyped as
...
Core function for calling Prima methods. Is used by following three 
functions, but never directly. Format is described in C<Calling perl code>
section.
...
Calls method to an object

=item sv_call_perl

 SV * ( SV * mate, char *subName, const char *format, ...); 

Calls method to a scalar
...
Destroys object. One of Prima function that can be called in 
re-entrant fashion.
...
Has no virtually use in C, only used in perl code.
...
Transfers argemants in perl stack to a newly created HV.
...
Created list instance with a static List structure.
...
Otherwise -1 is returned. action have to be function prototyped as
...
Returns the index of an item, or -1 if the item is not in the list.
