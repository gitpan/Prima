=for rcs $Id: gencls.pod,v 1.3 2002/05/14 13:22:32 dk Exp $
...
In short, the syntax of a .cls file can be desribed by the following scheme:
...
Gencls has several built-in scalar data types, that it knows how to work
with. To 'work' means that it can generate a code that transfers
...
that are mapped to int. The data undergo no conversion to int in trasfer
...
as the result is not a subject for -O optimization.
...
no substituion during .cls file processing is made.
...
be exactly as the dimensions af the arrays.
...
fixed name and paramater list. However, if the task of
...
function instead. The only request is that inetrnal()
...
This declaraion is used heavily in constructors,
...
defererencing of a C<Handle self> as a corresponding
structure:
...
menpage) interface. It is also expected that a C<public> method creates both
REDEFINED and FROMPERL functions ( see L<Prima::internals> manpage for
